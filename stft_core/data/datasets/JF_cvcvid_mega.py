...
from . import JF_CVCVIDImageDataset
from .cvcvid_image import CVCVIDImageDataset
from stft_core.config import cfg

class JF_CVCVIDMEGADataset(JF_CVCVIDImageDataset):
    def __init__(self, image_set, data_dir, img_dir, anno_path, img_index, transforms, is_train=True):
        super(JF_CVCVIDMEGADataset, self).__init__(image_set, data_dir, img_dir, anno_path, img_index,
    ...
    def _get_train(self, idx):
        ...
        img_refs_m = []
        img_refs_g = []
        if hasattr(self, "frames_vidDir_and_filename_pattern"):
    ...
    def _get_test(self, idx):
        ...
        if cfg.MODEL.VID.MEGA.GLOBAL.ENABLE:
            ...
            for id in range(size):
                filename = self.frames_vidDir_and_filename_pattern[idx] % shuffled_index[
